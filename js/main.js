...

{
  function setWord() {
    ...
    loc = 0;
    offAll()
    bright(fingerMaps[word.alphabet[loc].toLowerCase()])
    offAllKey()
    if(enableBright){
      brightKey(word.alphabet[loc].toLowerCase())
    }
  }
  ...
  document.addEventListener('keydown', e => {
    if (e.key.toLowerCase() !== word.alphabet[loc].toLowerCase()) {
      return;
    }


    inputWord += word.alphabet[loc].toLowerCase();
    ...
    oneWordAlphabet += word.alphabet[loc].toLowerCase();
